...
describe("CollectionHolder", () => {
  it("supports collection mutations and key predicates", () => {
    const holder = new CollectionHolder<Item>({ keyExtractor: value => value.id });
  ...
  it("loads, refreshes, extracts data and handles all result branches", async () => {
    ...
    await expect(holder.load()).resolves.toEqual({ data: [item(1)], error: null });
    await holder.refresh();
    await expect(
      holder.fromApi(async () => ({ data: { values: [item(2)] } }), value => value.values),
    ).resolves.toEqual({ data: [item(2)], error: null });
    await holder.fromApi(async () => ({ data: [item(3)] }), undefined, { refresh: true });
  ...
  it("covers every configured collection fetch result", async () => {
    const results = [cancelResponse, { error: { message: "api" } }, { data: null }];
    const holder = new CollectionHolder<Item>({ onFetch: async () => results.shift()! });
    ...
    const thrown = new CollectionHolder<Item>({ onFetch: async () => { throw new Error("boom"); } });

    await expect(thrown.load()).resolves.toMatchObject({ error: { message: "boom" } });
    const canceled = new CollectionHolder<Item>({ onFetch: async () => { throw cancelError; } });
