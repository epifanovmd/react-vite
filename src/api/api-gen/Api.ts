...
export class Api<E = unknown> extends HttpClient<E> {
  ...
  /**
   * @description Удалить сообщение. forAll=true — для всех, forAll=false — только для себя.
   *
   * @tags Message
   * @name DeleteMessage
  ...
  /**
   * @description Получить информацию о прочтении сообщения (кто прочитал, кто получил). Доступно для участников чата.
   *
   * @tags Message
   * @name GetReceiptInfo
   * @summary Информация о прочтении сообщения
   * @request GET:/api/message/{id}/receipts
   * @secure
   */
  getReceiptInfo = (
    { id, ...query }: GetReceiptInfoParams,
    params: RequestParams = {},
  ) =>
    this.request<MessageReceiptDto[]>({
  ...
  /**
   * @description Получить изменения с указанной версии (компактифицированные). Если версия клиента устарела — вернёт requiresSnapshot: true.
   *
   * @tags Sync
   * @name GetChanges
  ...
  /**
   * @description Получить текущую sync version. Используется при первом запуске для установки начальной точки синхронизации.
   *
   * @tags Sync
   * @name GetVersion
   * @summary Current sync version
   * @request GET:/api/sync/version
   * @secure
   */
  getVersion = (params: RequestParams = {}) =>
    this.request<ISyncVersionDto>({
