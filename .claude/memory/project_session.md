...
В вебе используется `CrossTabTokenStorage`: обёртка над постоянным
хранилищем, слушающая событие `storage`. Вход, обновление и выход в одной
вкладке доходят до остальных; собственные записи петлю не создают,
потому что событие приходит только в чужие вкладки.
